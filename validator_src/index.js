...
const { loadJsonFromStorage, logEventsToDatabase, DB_TABLE, STORAGE_BUCKET } = require('./helpers/supabaseHelpers.js');
...
exports.validateEvent = (req, res) => {
  ...
  if (!STORAGE_BUCKET || !DB_TABLE) {
    console.error('Missing configuration:', 'Bucket:', STORAGE_BUCKET, 'Table:', DB_TABLE);
  ...
  loadJsonFromStorage(`${eventName}.json`)
    .then((masterSchema) => {
      if (!masterSchema) {
      ...
      try {
        ...
        if (errorLogEntries.length > 0) {
          ...
          logEventsToDatabase(errorLogEntries);
          logEventsToDatabase(validFieldsEntries);
          ...
          if (validLogEntries.length > 0) {
            ...
            logEventsToDatabase(validLogEntries);
            logEventsToDatabase(validFieldsEntries);
    ...
    .catch((err) => {
      console.error('Error loading schema from storage:', err);
      return res.status(500).send({ status: 'error', message: 'Internal server error' });
    });
